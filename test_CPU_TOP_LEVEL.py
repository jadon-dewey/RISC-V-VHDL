...
class CPU_TOP_LEVEL(lib.Entity):
    ...
    control_if = lib.Entity.Output_pin # Check internal signal
    
    # New interrupt-related ports (if not generated internally)
    interrupt_req    = lib.Entity.Input_pin
    interrupt_addr   = lib.Entity.Input_pin
    mret             = lib.Entity.Input_pin
    acknowledge      = lib.Entity.Output_pin  # CPU sends an ack to the interrupt unit
...
@CPU_TOP_LEVEL.testcase
async def tb_CPU_TOP_LEVEL_interrupt_integration(dut: CPU_TOP_LEVEL, trace: lib.Waveform):
    """
    Integrated test for the interrupt unit:
      1. Run normal instructions and capture a baseline PC.
      2. Assert an interrupt (with a distinct interrupt vector) and verify that:
         - The hazard unit flushes the pipeline.
         - The PC jumps to the interrupt address.
         - The CPU generates an acknowledge.
      3. Simulate a return-from-interrupt by asserting mret, and verify that the PC
         restores to the saved value.
    """
    ...
    # Ensure no interrupt is active initially.
    dut.interrupt_req.value = BinaryValue("0")
    dut.mret.value = BinaryValue("0")
    
    # Assume data_program is loaded with a test program that, in normal operation,
    # causes the PC to increment normally.
    # Run a few cycles to capture a baseline PC.
    for _ in range(3):
        await trace.cycle()
    baseline_pc = dut.address_program.value.binstr

    #yield trace.check(dut.stage_wb_data_destination, baseline_pc,
                      #f"Before interrupt: baseline PC = {baseline_pc}")

    # ----- Interrupt Phase -----
    # Drive a distinct interrupt address
    dut.interrupt_addr.value = BinaryValue("00000000000000000000010000000000") # 400 in binary
    dut.interrupt_req.value  = BinaryValue("1")
    
    await trace.cycle()
    # After one cycle, the PC should jump to the interrupt address.
    yield trace.check(dut.address_program, "00000000000000000000010000000000",
                      "Interrupt active: PC should equal the interrupt address.")
    
    await trace.cycle()

    # Also, the CPU should assert acknowledge.
    yield trace.check(dut.acknowledge, "1", "CPU should acknowledge the interrupt.")
    
    # ----- Return-from-Interrupt Phase -----
    # Deassert interrupt and assert mret for one cycle.
    await trace.cycle()
    dut.interrupt_req.value = BinaryValue("0")
    ...
    # Now, the PC should restore to the saved baseline value (or an expected restored value).
    yield trace.check(dut.address_program, baseline_pc,
                      "Return-from-interrupt: PC should restore to the saved value after mret.")
...
@pytest.mark.testcases
def test_CPU_TOP_LEVEL_interrupt_testcases():
    CPU_TOP_LEVEL.test_with(tb_CPU_TOP_LEVEL_interrupt_integration)
