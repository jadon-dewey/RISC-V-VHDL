...
architecture RV32I of CPU_STAGE_ID is
    ...
    signal mret_control        : std_logic := '0';

begin

    -- Pipeline logic with or without register stage
    ...
    -- Enable flushing when jump or branch
    ...
    -- mret instruction decode
    process(source_0.data_instruction)
        variable instruction_word : std_logic_vector(31 downto 0);
    begin
        instruction_word := source_0.data_instruction;
        if instruction_word = x"30200073" then
            mret_control <= '1';
        else
            mret_control <= '0';
        end if;
    ...
    signals_ex.enable_multiplier <= signals_ex.control_wb.enable_multiplier;
    ...
    -- Optional Quartus-specific delay on PC forwarding
    ...
    -- Forward mret control bit
    signals_ex.mret <= mret_control;

    -- Branch calculation
    BRANCH_UNIT: entity WORK.MODULE_BRANCH_UNIT
        port map (
    ...
    -- Branch comparison
    BRANCH_COMPARE_UNIT: entity WORK.MODULE_BRANCH_COMPARE_UNIT
        port map (
