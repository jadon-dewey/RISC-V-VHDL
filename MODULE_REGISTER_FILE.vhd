...
begin
    ...
    -- **Prevent writing to register x0 (hardwired to zero)**
    process(select_source_1, select_source_2, select_destination)
    begin
        passthrough_1 <= '1' when (select_source_1 = select_destination and select_destination /= "00000") else '0';
        passthrough_2 <= '1' when (select_source_2 = select_destination and select_destination /= "00000") else '0';
    end process;
    ...
    GEN_REGISTERS : for i in 1 to (DATA_WIDTH - 1) generate
        REGISTER_I : entity WORK.GENERIC_REGISTER
            ...
            port map (
                clock       => clock,
                clear       => clear,
                enable      => enable AND (select_destination = std_logic_vector(to_unsigned(i, 5))),
                source      => data_destination,
                destination => registers(i)
            );
    end generate;

    -- **Read Logic for Source Register 1**
    MUX_DECODE_SOURCE_1 : entity WORK.GENERIC_MUX_32X1
        ...
        port map (
            ...
            source_1    => (others => '0'), -- Register x0 is always zero
    ...
    -- **Read Logic for Source Register 2**
