...
entity CPU_TOP_LEVEL is
    ...
    port (
        ...
        -- Mapping the new interrupt ports:
        interrupt_req   : in  std_logic;                    -- CPU receives interrupt request
        interrupt_addr  : in std_logic_vector(31 downto 0);  -- CPU takes the interrupt address as the input to branch to
        acknowledge     : out std_logic;                     -- CPU acknowledges the interrupt
        mret            : in  std_logic
    );
...
begin
    ...
    -- Edited so the CPU can branch to the interrupt address
    INSTRUCTION_FETCH : entity WORK.CPU_STAGE_IF(RV32I)
        port map (
            ...
            address_program => signals_if_id.address_program,
            interrupt_req   => interrupt_req,    -- New mapping for interrupt request
            interrupt_addr  => interrupt_addr,    -- New mapping for interrupt address
            mret            => mret
        );

    -- Added or flush_pipeline to every clear port, so that the instruction set will clear when an interrupt is asserted
