...
architecture RTL of RV32M_MULTIPLIER is
    ...
    signal operand_A : signed(32 downto 0);
    signal operand_B : signed(32 downto 0);
        ...
        if rising_edge(clock) then
            if enable = '1' then
                ...
                if select_fun = "00" then  -- MUL (Signed × Signed)
                    ...
                    operand_A <= signed('0' & input_A);
                    operand_B <= signed(std_logic_vector(to_unsigned(to_integer(unsigned(input_B)), 33)));
                else -- MULHU (Unsigned × Unsigned)
                    operand_A <= signed(std_logic_vector(to_unsigned(to_integer(unsigned(input_A)), 33)));
                    operand_B <= signed(std_logic_vector(to_unsigned(to_integer(unsigned(input_B)), 33)));
                end if;
